...
enum NeuralRenderingFusedWindowBlock {
  ...
  /// Windows per threadgroup for the single-head block (`MLXDLSS_WINDOW_BATCH`, 1 = one window).
  nonisolated(unsafe) static var windowsPerGroup: Int = {
    let value = Int(ProcessInfo.processInfo.environment["MLXDLSS_WINDOW_BATCH"] ?? "") ?? 1
  ...
  /// Several windows per threadgroup: the feed-forward weight chunks are staged
  /// once per group instead of once per window (`MLXDLSS_WINDOW_BATCH=N`).
  private static let batchedKernel = MLXFast.metalKernel(
    name: "mlxdlss_fused_window_block_1h32_batched",
    inputNames: [
      "input", "expansion", "projection", "feedForwardCosine", "qkvWeight",
      "attentionScale", "attentionBias", "attentionProjection", "attentionCosine", "params",
    ],
    ...
    source: #"""
      ...
      const bool publish = params[4] != 0u;
      const uint windowsX = (width + padLeft + 7) / 8;
      const uint windowCount = params[5];
      const uint firstWindow = threadgroup_position_in_grid.x * uint(windowsPerGroup);
      const uint groupWindows = min(uint(windowsPerGroup), windowCount - firstWindow);
      const device half4* input4 = (const device half4*)input;
      device half4* output4 = (device half4*)output;
      const device half4* expansion4 = (const device half4*)expansion;
      const device half4* projection4 = (const device half4*)projection;
      ...
      simdgroup_matrix<half, 8, 8> qt[2][4];
      simdgroup_matrix<half, 8, 8> at[2][4];
      // Feed-forward projection accumulators for every window of this group.
      simdgroup_matrix<half, 8, 8> pacc[windowsPerGroup][2][4];
      for (uint n = 0; n < uint(windowsPerGroup); ++n) {
        for (uint rr = 0; rr < 2; ++rr) {
          for (uint o = 0; o < 4; ++o) {
            pacc[n][rr][o].thread_elements()[0] = 0.0h;
            pacc[n][rr][o].thread_elements()[1] = 0.0h;
          }
        }
      }

      // Phase 1: feed-forward for every window of the group; each staged weight
      // chunk serves all of them (the accumulation order per window is unchanged).
      for (uint chunk = 0; chunk < 2; ++chunk) {
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint i = tid; i < 512; i += 128) {
          W4[i] = expansion4[(i / 16) * 32 + chunk * 16 + i % 16];
        ...
        for (uint n = 0; n < uint(windowsPerGroup); ++n) {
          if (n >= groupWindows) { break; }
          const uint windowIndex = firstWindow + n;
          const uint windowY = windowIndex / windowsX;
          const uint windowX = windowIndex % windowsX;
          const int rowY0 = int(windowY * 8) - int(padTop);
          const int colX0 = int(windowX * 8) - int(padLeft);
          ...
          for (uint rr = 0; rr < 2; ++rr) {
            ...
            for (uint o = 0; o < 4; ++o) {
              for (uint k = 0; k < 8; ++k) {
                ...
                simdgroup_load(left, H + k * 8, 64, ulong2(0), false);
                simdgroup_load(right, W + 2048 + k * 8 * 32 + o * 8, 32, ulong2(0), false);
                simdgroup_multiply_accumulate(pacc[n][rr][o], left, right, pacc[n][rr][o]);
              }
            }
            simdgroup_barrier(mem_flags::mem_threadgroup);
          }
      ...
      // Attention for every window in turn; the shared K/V region is reused.
      _Pragma("clang loop unroll(full)")
      for (uint n = 0; n < uint(windowsPerGroup); ++n) {
        if (n >= groupWindows) { break; }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        for (uint rr = 0; rr < 2; ++rr) {
          for (uint o = 0; o < 4; ++o) {
            simdgroup_store(pacc[n][rr][o], T + o * 8, 32, ulong2(0), false);
            simdgroup_store(xt[rr][o], T + 256 + o * 8, 32, ulong2(0), false);
          }
          simdgroup_barrier(mem_flags::mem_threadgroup);
          for (uint i = lane; i < 64; i += 32) {
...
"""#,
  ...
  static func apply(
    ...
    let inputs = [
      ...
      half(attentionProjectionWeight), half(attentionCosine),
    ]
    let batch = windowsPerGroup
    if batch > 1 {
      let groups = (windowCount + batch - 1) / batch
      let params = NeuralRenderingKernelParameters.array([
        UInt32(height), UInt32(width), UInt32(padTop), UInt32(padLeft), publish ? UInt32(1) : UInt32(0), UInt32(windowCount),
      ])
      return batchedKernel(
        inputs + [params],
        template: [("windowsPerGroup", batch)],
        grid: (groups * 32 * simdgroupsPerWindow, 1, 1),
